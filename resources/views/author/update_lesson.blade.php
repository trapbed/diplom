...
@section('title', "Редактирование урока '{{ $data->title }}'")
@section('content')
    ...
    <span class="ff_mr fsz_2">Редактировать {{ $data->type == 'lesson' ? 'урок' : 'тестирование'}} к : '{{$course->title}}'</span>
    ...
    <form id="preview" class="sc_w_th oy_s h30 df fdr_c g1" action="{{route('update_lesson')}}" method="POST" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="id_lesson" value="{{ $data->id }}">
        <input type="hidden" name="id_course" value="{{$course->id}}">
        <label class="fsz_1_2 ff_mr c_dp" for="title">Заголовок урока</label>
        <input id="title_lesson" class="w48 h1 paa_0_5 ff_mr fsz_1 br_03 brc_lp ou_n" value="{{ $data->title }}" type="text" name="title">
        @if ($data->type == 'lesson')
            <div class="one_lesson_update">
                @csrf
                @foreach (json_decode($data->content, TRUE) as $point=>$cont)
                    @foreach ($cont as $key=>$val)
                        @if ($key == 'txt')
                            <div class="pos_r w50 wmx_50 ou_n" data-id="{{ $point }}">
                                <textarea required name="text[{{ $point }}]" class="input_create paa_0_3 ff_mr fsz_1 ou_n">{{ $val }}</textarea>
                                <div id="minus_elem{{ $point }}" onclick="minus_elem('minus_elem{{ $point }}')" class="pos_a b_3 rm_5 fsz_2 paa_1 bg_dp br_03 c_lp minus_elem">-</div>
                            </div>
                        @elseif($key == 'img')
                            <input type="hidden" id="img_{{ $point }}" value="{{ $val }}" name="img[{{ $point }}]" class="dn img_{{ $point }} images">
                            <div class="w50 pos_r img_{{ $point }}">
                                <img class="w50" src="{{asset( 'img/lessons/'.$val) }}" class="img_{{ $point }}">
                                <div id="del_img_{{ $point }}" onclick="minus_img('img_{{ $point }}', this)" class="img_{{ $point }} pos_a t_1 rm_5 fsz_2 paa_1 bg_dp br_03 c_lp minus_elem">-</div>
                            </div>
                        @endif
                        <br>
                    @endforeach
    ...
    <script>
        ...
        all_count = {{ count(json_decode($data->content, TRUE)) }};
    ...
    {{--@if ($data->type == 'lesson')
        <div class="one_lesson_update">
            <form action="" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="text" name="" value="{{ $data->title }}">
                @foreach (json_decode($data->content, TRUE) as $point=>$cont)
                    @foreach ($cont as $key=>$val)
                        @if ($key == 'txt')
                            <textarea name="text[{{ $point }}]" id="">
                                {{ $val }}
                            </textarea>
                        @elseif($key == 'img')
                            <img class="w32" src="{{ asset('img/lessons/'.$val) }}" alt="">
